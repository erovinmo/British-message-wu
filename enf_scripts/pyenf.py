...
class pyENF:
    def __init__(
        ...
        self.harmonic_multiples = self.harmonic_multiples[
            1 : len(self.harmonic_multiples)
        ]  # multiple harmonics to combine them and get better signal estimate
    ...
    def compute_spectrogam_strips(self):
        ...
        spectro_strips = (
            []
        )  # collecting the psd strips regarding each selected frequency range around nominal freq
        ...
        number_of_frames = math.ceil(
            (length_signal - frame_size + 1) / shift_amount
        )  # based on given frame size
        ...
        win = signal.get_window(
            "hamming", frame_size
        )  # creating a hamming window for each frame segment
        for frame in range(number_of_frames):
            ending = starting + frame_size
            x = self.signal0[starting:ending]
            f, t, P = signal.spectrogram(
                x,
                window=win,
                noverlap=self.overlap_amount_secs,
                nfft=self.nfft,
                fs=self.fs,
                mode="psd",
            )
    ...
    def compute_combining_weights_from_harmonics(self):
        number_of_duration = math.ceil(
            len(self.signal0) / (self.duration * 60 * self.fs)
        )
        frame_size = math.floor(self.frame_size_secs * self.fs)
        overlap_amount = self.overlap_amount_secs * self.fs
        shift_amount = frame_size - overlap_amount
        ...
        for dur in range(number_of_duration - 1):
            ...
            x = self.signal0[
                int(dur * self.duration * 60 * self.fs) : int(
                    min(
                        len(self.signal0),
                        ((dur + 1) * self.duration * 60 * self.fs + overlap_amount),
                    )
                )
            ]
            ...
            number_of_frames = math.ceil(
                (len(x) - frame_size + 1) / shift_amount
            )  # based on given frame size
            # Collecting the spectrogram strips for each window in the signal and storing them in the list
            # The rows change based on the nfft selected. It also effects the resolution of the frequency range
            rows = int(self.nfft / 2 + 1)
            ...
            width_init = self.find_closest(f, center_frequency) - self.find_closest(
                f, starting_frequency
            )
            HarmonicStrips = np.zeros(
                shape=(
                    (width_init * 2 * sum(self.harmonic_multiples)),
                    number_of_frames,
                )
            )
            FreqAxis = np.zeros(
                shape=((width_init * 2 * sum(self.harmonic_multiples)), 1)
            )
            resolution = f[1] - f[0]
            ...
            for k in range(number_of_harmonics):
                currStrip = HarmonicStrips[
                    int(starting_indices[k]) : int(ending_indices[k]), :
                ]
                ...
                outside_mean[k, dur] = np.mean(
                    np.append(outside_strip1, outside_strip2)
                )
    ...
    def compute_combined_spectrum(self, strips, weights, freq_support):
        ...
        for dur in range(number_of_duration):
            number_of_frames_left = (
                number_of_frames - dur * number_of_frames_per_duration
            )
            OurStrip = np.zeros(
                shape=(
                    int(strip_width),
                    min(int(number_of_frames_per_duration), int(number_of_frames_left)),
                )
            )
            ...
            for harm in range(number_of_signals):
                ...
                for frame in range(q):
                    ...
                    tempo = np.array(
                        Image.fromarray(obj=temp).resize(
                            size=(1, strip_width), resample=Image.BILINEAR
                        )
                    )
                    # print(tempo)
                    tempo = 100 * tempo / max(tempo)
                    OurStrip[:, frame : (frame + 1)] = OurStrip[
                        :, frame : (frame + 1)
                    ] + (weights[harm, dur] * tempo)
    ...
    def compute_ENF_from_combined_strip(self, OurStripCell, initial_frequency):
        ...
        number_of_frames = (
            number_of_frames_per_dur * (number_of_duration - 1)
            + ((OurStripCell[0]).shape)[1]
        )
